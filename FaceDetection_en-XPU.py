...
class PersonDialog(BaseDialog):
    def __init__(self, parent, image, face_location, existing_persons=None, ref_persons=None, db_path=None):
        ...
        # --- MODIFIED: Ensure existing_persons and ref_persons are used for Treeviews ---
        if self.existing_persons:
            ...
            columns = ('ID', 'Full Name', 'Short Name', 'Notes'); # Added Notes column
            self.person_tree = ttk.Treeview(tree_frame, columns=columns, show='headings', height=8)
            for col in columns:
                self.person_tree.heading(col, text=col)
                if col == 'ID': self.person_tree.column(col, width=50, anchor='center')
                elif col == 'Full Name': self.person_tree.column(col, width=150)
                elif col == 'Short Name': self.person_tree.column(col, width=100)
                elif col == 'Notes': self.person_tree.column(col, width=200) # Set width for Notes
            ...
            # Populate the treeview with data from existing_persons
            for person in self.existing_persons:
                self.person_tree.insert('', tk.END, values=(person['id'], person['full_name'], person['short_name'], person.get('notes', ''))) # Added .get for notes to avoid KeyError
        ...
        if self.ref_persons:
            ...
            columns = ('ID', 'Full Name', 'Short Name', 'Notes'); # Added Notes column
            self.ref_person_tree = ttk.Treeview(ref_tree_frame, columns=columns, show='headings', height=8)
            for col in columns:
                self.ref_person_tree.heading(col, text=col)
                if col == 'ID': self.ref_person_tree.column(col, width=50, anchor='center')
                elif col == 'Full Name': self.ref_person_tree.column(col, width=150)
                elif col == 'Short Name': self.ref_person_tree.column(col, width=100)
                elif col == 'Notes': self.ref_person_tree.column(col, width=200) # Set width for Notes
            ...
            # Populate the treeview with data from ref_persons
            for person in self.ref_persons:
                self.ref_person_tree.insert('', tk.END, values=(person['id'], person['full_name'], person['short_name'], person.get('notes', ''))) # Added .get for notes
...
class DogDialog(BaseDialog):
    def __init__(self, parent, image, dog_bbox, existing_dogs=None, ref_dogs=None, db_path=None, breed=None):
        ...
        # --- MODIFIED: Ensure existing_dogs and ref_dogs are used for Treeviews ---
        if self.existing_dogs:
            existing_frame = ttk.Frame(self.notebook); self.notebook.add(existing_frame, text="Select from DB")
            tree_frame = ttk.Frame(existing_frame, padding="10"); tree_frame.pack(fill=tk.BOTH, expand=True)
            columns = ('ID', 'Name', 'Breed', 'Owner', 'Notes'); # Added Notes column
            self.dog_tree = ttk.Treeview(tree_frame, columns=columns, show='headings', height=8)
            for col in columns:
                self.dog_tree.heading(col, text=col)
                if col == 'ID': self.dog_tree.column(col, width=50, anchor='center')
                elif col == 'Name': self.dog_tree.column(col, width=120)
                elif col == 'Breed': self.dog_tree.column(col, width=120)
                elif col == 'Owner': self.dog_tree.column(col, width=120)
                elif col == 'Notes': self.dog_tree.column(col, width=150) # Set width for Notes
            ...
            # Populate the treeview with data from existing_dogs
            for dog in self.existing_dogs:
                self.dog_tree.insert('', tk.END, values=(dog['id'], dog['name'], dog.get('breed', 'N/A'), dog.get('owner', 'N/A'), dog.get('notes', ''))) # Added .get for notes
        ...
        if self.ref_dogs:
            ...
            ref_tree_frame = ttk.Frame(ref_frame, padding="10"); ref_tree_frame.pack(fill=tk.BOTH, expand=True)
            columns = ('ID', 'Name', 'Breed', 'Owner', 'Notes'); # Added Notes column
            self.ref_dog_tree = ttk.Treeview(ref_tree_frame, columns=columns, show='headings', height=8)
            for col in columns:
                self.ref_dog_tree.heading(col, text=col)
                if col == 'ID': self.ref_dog_tree.column(col, width=50, anchor='center')
                elif col == 'Name': self.ref_dog_tree.column(col, width=120)
                elif col == 'Breed': self.ref_dog_tree.column(col, width=120)
                elif col == 'Owner': self.ref_dog_tree.column(col, width=120)
                elif col == 'Notes': self.ref_dog_tree.column(col, width=150) # Set width for Notes
            
            ref_tree_scroll = ttk.Scrollbar(ref_tree_frame, orient="vertical", command=self.ref_dog_tree.yview)
            self.ref_dog_tree.configure(yscrollcommand=ref_tree_scroll.set)
            
            # Populate the treeview with data from ref_dogs
            for dog in self.ref_dogs:
                self.ref_dog_tree.insert('', tk.END, values=(dog['id'], dog['name'], dog.get('breed', ''), dog.get('owner', ''), dog.get('notes', ''))) # Added .get for notes
...
class BodyWithoutFaceDialog(BaseDialog):
    def __init__(self, parent, image, body_bbox, existing_persons=None, ref_persons=None, db_path=None):
        ...
        if self.existing_persons:
            ...
            # Populate the treeview with data from existing_persons
            for person in self.existing_persons:
                self.person_tree.insert('', tk.END, values=(person['id'], person['full_name'], person['short_name'], person.get('notes', ''))) # Added .get for notes
...
class FaceDetectionV2:
    def __init__(self, root):
        ...
        if torch.xpu.is_available():          # Intel® GPU / XPU
            self.dog_device = "xpu"
        elif torch.cuda.is_available():       # NVIDIA CUDA
            self.dog_device = "cuda"
        else:
            self.dog_device = "cpu"
        ...
        if not self.processing:
            ...
            try:
                ...
                if self.yolo is None or self.loaded_yolo_model_name != self.yolo_model.get():
                    ...
                    try: 
                        self.yolo.to(self.dog_device)            # <-- GPU if supported
                        self.log(f"YOLO model successfully moved to {self.dog_device.upper()}.")
                    except Exception as e:
                        self.log(f"Warning: Could not move YOLO model to {self.dog_device.upper()}: {e}. Falling back to CPU.")
                        self.yolo.cpu() # Explicitly move to CPU if the above fails
    ...
    def refresh_people_list(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("""
                    SELECT p.id,
                           CASE WHEN p.is_known THEN 'Known' ELSE 'Unknown' END AS status,
                           p.full_name,
                           p.short_name,
                           COUNT(DISTINCT pd.image_id) AS photos,
                           p.notes
                    FROM persons p
                    LEFT JOIN person_detections pd ON p.id = pd.person_id
                    GROUP BY p.id
                    ORDER BY p.is_known DESC, p.full_name
                """)
                for row in cursor.fetchall():
                    self.people_tree.insert('', tk.END, values=(row['id'], row['status'], row['full_name'], row['short_name'], row['photos'], row['notes']))
        except Exception as e:
            self.log(f"Error refreshing people list: {e}")
    ...
    def refresh_dogs_list(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.row_factory = sqlite3.Row # Ensure rows are dict-like
                cursor = conn.cursor()
                cursor.execute("SELECT d.id, CASE WHEN d.is_known THEN 'Known' ELSE 'Unknown' END AS status, d.name, d.breed, d.owner, COUNT(DISTINCT dd.image_id) AS photos, d.notes FROM dogs d LEFT JOIN dog_detections dd ON d.id = dd.dog_id GROUP BY d.id ORDER BY d.is_known DESC, d.name")
                for row in cursor.fetchall(): self.dogs_tree.insert('', tk.END, values=(row['id'], row['status'], row['name'], row['breed'], row['owner'], row['photos'], row['notes']))
    ...
    # --- MODIFIED: get_existing_persons to retrieve ALL known persons ---
    def get_existing_persons(self, db_path=None):
        ...
        try:
            with sqlite3.connect(f'file:{target_db}?mode=ro', uri=True) as conn:
                ...
                cursor = conn.cursor()
                # Select all known persons, regardless of whether they have face encodings
                cursor.execute('SELECT id, full_name, short_name, notes FROM persons WHERE is_known = 1 ORDER BY full_name')
                results = [dict(row) for row in cursor.fetchall()]
                self.log(f"DEBUG: get_existing_persons from {os.path.basename(target_db)} returned {len(results)} known persons.") # Debug log
                return results
        except Exception as e: self.log(f"Error reading list of people from {os.path.basename(target_db)}: {e}"); return []

    # --- MODIFIED: get_existing_dogs to retrieve ALL known dogs ---
    def get_existing_dogs(self, db_path=None):
        ...
        try:
            with sqlite3.connect(f'file:{target_db}?mode=ro', uri=True) as conn:
                ...
                results = [dict(row) for row in cursor.fetchall()]
                self.log(f"DEBUG: get_existing_dogs from {os.path.basename(target_db)} returned {len(results)} known dogs.") # Debug log
                return results
    ...
    def show_person_dialog_main(self, data):
        ...
        # Pass ALL known persons from main DB
        existing_persons = self.get_existing_persons(db_path=self.db_path)
        # Pass ALL known persons from reference DB
        ref_persons = self.get_existing_persons(db_path=self.ref_db_path) if self.ref_db_path else []
        self.log(f"DEBUG: PersonDialog called with {len(existing_persons)} existing persons and {len(ref_persons)} ref persons.") # Debug log
        dialog = PersonDialog(self.root, image, face_location, existing_persons, ref_persons, self.db_path)
        self.root.wait_window(dialog); callback(dialog.result)

    def show_dog_dialog_main(self, data):
        image, dog_bbox, callback, breed = data
        # Pass ALL known dogs from main DB
        existing_dogs = self.get_existing_dogs(db_path=self.db_path)
        # Pass ALL known dogs from reference DB
        ref_dogs = self.get_existing_dogs(db_path=self.ref_db_path) if self.ref_db_path else []
        self.log(f"DEBUG: DogDialog called with {len(existing_dogs)} existing dogs and {len(ref_dogs)} ref dogs.") # Debug log
        dialog = DogDialog(self.root, image, dog_bbox, existing_dogs, ref_dogs, self.db_path, breed=breed)
        self.root.wait_window(dialog); callback(dialog.result)
    ...
    def show_body_dialog_main(self, data):
        ...
        # Pass ALL known persons from reference DB
        ref_persons = self.get_existing_persons(db_path=self.ref_db_path) if self.ref_db_path else []
        self.log(f"DEBUG: BodyWithoutFaceDialog called with {len(existing_persons)} existing persons and {len(ref_persons)} ref persons.") # Debug log
    ...
    def create_or_update_person(self, result, conn):
        """
        Создаёт новую или обновляет существующую запись в таблице persons
        и возвращает person_id.  После успешного добавления кладёт событие
        'refresh_people' в update_queue, чтобы интерфейс сразу показал нового человека.
        """
        cursor = conn.cursor()
        ...
        if result['action'] == 'new_known':
            ...
            self.update_queue.put(('refresh_people', None))   # <— обновляем список

        # 2. «Save information» из окна Body‑without‑face
        # ЭТА ЧАСТЬ НУЖДАЕТСЯ В ИСПРАВЛЕНИИ, ЧТОБЫ УСТАНАВЛИВАТЬ is_known = 1
        elif result['action'] == 'local_known': # Действие "Save Information" из BodyWithoutFaceDialog
            cursor.execute(
                'INSERT INTO persons (is_known, full_name, short_name, notes, created_date, updated_date) '
                'VALUES (1, ?, ?, ?, ?, ?)', # Изменено с 0 на 1
                (result['full_name'], result['short_name'], result['notes'], now, now)
            )
            person_id = cursor.lastrowid
        ...
        # 3. «Leave as Unknown» (Это должно оставаться is_known = 0)
            ...
            # неизвестных в список не показываем → очередь не трогаем

        # 4. Выбор существующего человека
        elif result['action'] == 'existing':
            person_id = result['person_id']
            self.log(f"  Selected existing person (ID: {person_id})")
        # 5. Выбор существующего человека из референсной БД
        elif result['action'] == 'existing_ref': # Добавлена обработка для existing_ref
            # Эта логика должна быть уже обработана в _identify_person
            # если сюда доходит, это, возможно, ошибка или дублирование
            # лучше всего убедиться, что get_or_create_person_by_name вызывается там, где нужно
            # и возвращает person_id, который уже is_known=1
            pass
    ...
    def create_or_update_dog(self, result, conn):
        ...
        if result['action'] == 'new_known':
            cursor.execute('INSERT INTO dogs (is_known, name, breed, owner, notes, created_date, updated_date) VALUES (1, ?, ?, ?, ?, ?, ?)', (result['name'], result.get('breed',''), result.get('owner',''), result.get('notes',''), now, now))
            dog_id = cursor.lastrowid
            self.log(f"  Created new dog: {result['name']} (ID: {dog_id})")
            ...
            dog_id = cursor.lastrowid
            self.log(f"  Added an unknown dog (ID: {dog_id})")
        elif result['action'] == 'existing':
            dog_id = result['dog_id']
            self.log(f"  Selected existing dog (ID: {dog_id})")
        return dog_id
...
def main():
    ...
    print("Torch device chosen:", app.dog_device)
    print("XPU available      :", torch.xpu.is_available())
    if app.dog_device == "xpu":
        print("Current XPU name   :", torch.xpu.get_device_name(0))
    root.mainloop()
