"""
Face Detection v3.9.0
Программа для распознавания и учета людей и собак на фотографиях

Версия: 3.9.0
- Интегрирована система распознавания собак из DogRecognizerCPU (на базе Torchvision).
  - Детекция собак теперь выполняется моделью Faster R-CNN, а классификация
    пород - DenseNet-121. Это значительно повысило точность.
- В диалоге добавления собаки поле "Порода" теперь автоматически заполняется
  на основе данных классификатора.
- Добавлена индикация загрузки моделей при первом запуске:
  - Для моделей Torchvision в логе отображается прогресс-бар в процентах.
  - Для моделей YOLO выводится предупреждение о начале загрузки.
- YOLO теперь используется только для детекции людей.

Версия: 3.8.0
- Добавлены раздельные ползунки для настройки порога уверенности YOLO
  отдельно для людей и для собак.
- Обновлена логика обработки для поддержки индивидуальных порогов.
"""
...
# Новые импорты для распознавания собак
import torch
...
# --- Класс для перенаправления вывода в лог (для прогресс-бара) ---
class StdOutRedirector:
    ...
    def write(self, text):
        # Пишем в GUI только если в тексте есть перевод строки или возврат каретки
        # Это нужно, чтобы прогресс-бар torchvision обновлялся в одной строке
...
# ... (Классы ProcessedImageDialog, PersonDialog, BodyWithoutFaceDialog, ConfirmPersonDialog остаются без изменений) ...
...
class DogDialog(BaseDialog):
    # ИЗМЕНЕНИЕ: Добавлен параметр 'breed' для автозаполнения
    def __init__(self, parent, image, dog_bbox, existing_dogs=None, ref_dogs=None, db_path=None, breed=None):
        ...
        # ИЗМЕНЕНИЕ: Устанавливаем породу, если она была передана
        if breed:
...
class FaceDetectionV2:
    def __init__(self, root):
        ...
        # ИЗМЕНЕНИЕ: Порог для YOLO теперь только для людей
        self.yolo_person_conf = tk.DoubleVar(value=0.5)
        # Убран порог для собак
        ...
        # --- Новые атрибуты для моделей собак ---
        ...
        self.dog_detection_threshold = tk.DoubleVar(value=0.5)
        # --- --- ---
        ...
        # Перенаправляем stdout/stderr для отлова прогресс-бара
        self.stdout_redirector = StdOutRedirector(self.update_queue)
        
        self.process_queue()
        self.update_status(f"Готов к работе. Устройство для Torch: {self.dog_device.upper()}", 'idle')
    ...
    def create_scan_tab(self, parent):
        ...
        ttk.Label(dir_frame, text="Модель YOLO (для людей):").grid(row=5, column=0, sticky=tk.W, pady=5)
        ...
        ttk.Label(dir_frame, text="Порог YOLO для людей:").grid(row=6, column=0, sticky=tk.W, pady=5)
        ...
        ttk.Label(person_conf_frame, text="(чем выше значение, тем строже отбор)", font=('Arial', 9, 'italic')).pack(side=tk.LEFT)
        
        # ИЗМЕНЕНИЕ: Ползунок для порога собак заменен на ползунок для нового детектора
        ttk.Label(dir_frame, text="Порог детекции собак (Torch):").grid(row=7, column=0, sticky=tk.W, pady=5)
        ...
        ttk.Label(dog_conf_frame, text="(вероятность, что объект - собака)", font=('Arial', 9, 'italic')).pack(side=tk.LEFT)

        # --- Блок 3: Поведение ---
        processed_frame = ttk.LabelFrame(dir_frame, text="Повторная обработка", padding="10")
        processed_frame.grid(row=8, column=0, columnspan=4, sticky="ew", pady=10, padx=5) # Изменен row
    ...
    def init_database(self, db_path):
        ...
        try:
            ...
            with sqlite3.connect(db_path) as conn:
                ...
                add_column_if_not_exists('dog_detections', 'breed_source', 'TEXT') # Для породы
    ...
    def init_dog_models(self):
        """Инициализирует модели Torchvision для собак, отображая прогресс-бар."""
        if self.dog_det_model and self.dog_cls_model:
            return True
        self.log(f"Загрузка моделей для собак на устройство: {self.dog_device.upper()}...")
        
        original_stdout = sys.stdout
        sys.stdout = self.stdout_redirector
        try:
            self.update_status("Загрузка моделей для собак...", 'processing')
            # Детектор
            ...
            self.log("Модели для собак успешно загружены.")
            return True
        except Exception as e:
            self.log(f"Ошибка загрузки моделей Torchvision: {e}")
            return False
        finally:
            sys.stdout = original_stdout # Возвращаем стандартный вывод
    ...
    def start_processing(self):
        ...
        if not self.processing:
            ...
            try:
                # Инициализация моделей для собак
                if not self.init_dog_models():
                    raise Exception("Не удалось загрузить модели для собак.")

                # Инициализация YOLO
                if self.yolo is None or self.loaded_yolo_model_name != self.yolo_model.get():
                    model_file = Path(self.yolo_model.get())
                    if not model_file.exists():
                        self.log(f"Модель YOLO '{model_file}' не найдена. Начинается загрузка...")
                        self.log("Это может занять некоторое время, программа не зависла.")
                        self.update_status(f"Загрузка YOLO: {model_file}...", 'processing')
                ...
                self.log(f"Ошибка инициализации моделей: {e}")
                self.processing = False
    ...
    def show_dog_dialog_main(self, data):
        image, dog_bbox, callback, breed = data # Добавлен параметр breed
        ref_dogs = self.get_existing_dogs(self.ref_db_path) if self.ref_db_path else []
        # Передаем породу в диалог
    ...
    def detect_dogs_torchvision(self, pil_image):
        """Обнаруживает собак и их породы с помощью моделей Torchvision."""
        self.log("  Запуск детекции собак (Torchvision)...")
        DOG_COCO_CLASS = 18 # Класс "dog" в датасете COCO
        threshold = self.dog_detection_threshold.get()
        
        # Инференс детектора на полном изображении
        with torch.no_grad():
            # unsqueeze(0) добавляет batch dimension
            out = self.dog_det_model(self.dog_prep_det(pil_image).unsqueeze(0).to(self.dog_device))[0]

        # Отбираем только собак с уверенностью выше порога
        detected_dogs = []
        for i, (box, label, score) in enumerate(zip(out["boxes"], out["labels"], out["scores"])):
            if label.item() == DOG_COCO_CLASS and score.item() >= threshold:
                # Обрезаем изображение под найденную собаку
                ...
                detected_dogs.append({
                    ...
                    'breed': breed.split(',')[0].capitalize(), # Берем первое, самое общее название
                    'breed_confidence': breed_p
                })
        self.log(f"  Torchvision обнаружил: {len(detected_dogs)} собак.")
    ...
    def analyze_image(self, image_path, image_id, conn):
        try:
            ...
            # --- Этап 1: Детекция людей (YOLO) и лиц (face_recognition) ---
            self.log("  Запуск детекции людей (YOLO) и лиц (FaceRec)...")
            
            # Ищем только людей (class 0)
            results = self.yolo(image, conf=self.yolo_person_conf.get(), classes=[0]) 
            ...
            self.log(f"  YOLO обнаружил: {len(person_detections)} людей.")
            ...
            # ... (логика связывания лиц с телами остается прежней) ...
            unmatched_faces = []
            ...
            # ... (логика сортировки и идентификации людей остается прежней) ...
            def get_sort_key(p_obj):
                has_face = p_obj.get('has_face', False)
                area = (p_obj['bbox'][2] - p_obj['bbox'][0]) * (p_obj['bbox'][3] - p_obj['bbox'][1])
                return (1 if has_face else 0, area)
            
            all_people_to_process.sort(key=get_sort_key, reverse=True)
            self.log("  Объекты людей отсортированы.")
            ...
            # --- Этап 2: Детекция собак (Torchvision) ---
            ...
            for dog in dog_detections:
                ...
                if res := dialog_result.get('result'):
                    if res['action'] == 'existing_ref': 
                        ...
                        # Если создаем новую собаку, подставляем породу из детектора
                        if res['action'] == 'new_known' and not res.get('breed'):
                            res['breed'] = dog['breed']
                        dog['dog_id'] = self.create_or_update_dog(res, conn)
            
            # --- Этап 3: Аннотация и возврат результатов ---
            ...
            for dog in dog_detections:
                ...
                label = f"{name} ({breed_text})" if name != "Неизвестный" else breed_text
        ...
        except Exception as e: self.log(f"Критическая ошибка при анализе изображения: {e}\n{traceback.format_exc()}"); return 0, 0, 0, [], None, []
        ...
        try:
            ...
            for i, image_path in enumerate(image_files):
                ...
                with sqlite3.connect(self.db_path, timeout=10) as conn:
                    ...
                    # ИЗМЕНЕНИЕ: вызывается новый общий метод анализа
...
if __name__ == "__main__":
    # Для корректной работы PyTorch на Windows в мультипроцессном/многопоточном окружении
